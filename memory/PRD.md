...
- **✅ ÇÖZÜLDÜ 2026-06 (görselli, "bozmadan yap")**: Overlap sorunu **PROFESYONEL temada**. Kök neden: Neural Link paneli kullanıcı tarafından yeniden boyutlandırılabilir/dock edilebilir (varsayılan 440px) ama tema sabit `right: 360` bırakıyordu → panel içeriğin üstüne ~80px biniyordu. ÇÖZÜM (`ProfesyonelInterface.jsx`): sabit 360 kaldırıldı; konteyner insetleri Sidebar'ın yayınladığı gerçek CSS değişkenlerine bağlandı — `top/right/left/bottom: var(--sx-sb-top/right/left/bottom, 0px)` (mobilde right:0/bottom:64). Böylece panel hangi kenara dock edilirse edilsin ve hangi genişlikte olursa olsun içerik tam sığar. Doğrulama: Playwright metrik — profRight=1480 === sidebarLeft=1480 (vw 1920, sidebar 440) → 0px binme. Detaylı'ya DOKUNULMADI.
- **⚠️ NOT (aynı latent bug diğer temalarda)**: `KolayInterface.jsx`, `TeknikInterface.jsx`, `AydinlikInterface.jsx`, `PanoInterface.jsx` hâlâ sabit `right: 360` kullanıyor → aynı binme onlarda da olabilir. Kullanıcı yalnızca Profesyonel'i istedi; diğerlerine AYNI CSS-var düzeltmesi "YAP" deyince uygulanacak.
- **✅ PROFESYONEL: görev tıkla → düzenle + grafikler (2026-06, "YAP")** (`ProfesyonelInterface.jsx`): (a) görev kartlarına `onClick → setEditing(t)` → mevcut `EditTaskModal` Profesyonel üstünde açılıyor (Detaylı'ya ATMADAN), `onSave → tasksApi.update + load()`. (b) Panele iki mini recharts grafik: `prof-chart-catdist` (iş koluna göre aktif görev dağılımı, bar, top 6) + `prof-chart-trend` (son 7 gün tamamlanan, çizgi). Renk `rgb(var(--sx-accent-rgb))`. Screenshot ile doğrulandı. Detaylı'ya dokunulmadı.
- **✅ PROFESYONEL: grafiğe tıkla-filtrele (2026-06, "YAP")**: `catFilter` state; catdist bar'ına tıklayınca görev listesi o iş koluna filtreleniyor (aynı bara tekrar tıkla = toggle kapat). Seçili bar açık renkli (`Cell`), GÖREVLER başlığının yanında "<isim> ✕" çipi (`prof-catfilter-clear`) → tıkla=temizle. "Kolsuz" barı = iş kolu olmayan görevler. "tıkla → filtrele" ipucu grafik başlığında. Doğrulama: 21→17 filtre, çip "Kolsuz", temizle→21.
- **✅ PROFESYONEL: sonuç sayısı rozeti (2026-06, "YAP")**: GÖREVLER başlığının sağında `prof-result-count` → `{visibleTasks.length} {q varsa "sonuç" yoksa "görev"}`. Filtre+arama zaten birlikte çalışıyordu (visibleTasks catFilter+q); rozet anlık güncelleniyor. Doğrulama: "21 görev" → "seri" ara → "0 sonuç".
- **📝 Dock hafızası = ZATEN ÇALIŞIYOR**: kullanıcı "nereye koyduysam orada açılıyor" dedi → yeni iş yok (`sertex_sidebar_dock_v1`).
- **✅ İş kolu SIRALAMA — YAPILDI (2026-06, "hiçbir şeyi bozmadan yap")**: `categoryTree.js` → yeni `flattenCategoryOptions(categories)` yardımcısı (flattenTree DFS + nbsp girinti + "↳" alt-kol işareti). `TemplateFormModal.jsx` ve `EditTaskModal.jsx` native `<select>`'leri bu yardımcıya çevrildi (EditTaskModal çoklu-şirket optgroup dahil, her grup kendi içinde `flattenTree(bucket.items)`). Artık ana kol A→Z + alt kollar parent'ının hemen altında girintili. Yeni Görev formu zaten CategorySelect+flattenTree kullandığı için dokunulmadı. Doğrulama: kullanıcının resmindeki veriyle Node birim testi → beklenen sıra birebir. Detaylı LAYOUT'una dokunulmadı (yalnızca dropdown seçenek sırası düzeldi).
...
## ✅ TEKNİK + AYDINLIK ARAYÜZ TAM PARİTE (2026-06 · fork) TAMAMLANDI
Kullanıcı isteği: `TeknikInterface` ve `AydinlikInterface` (şimdiye kadar sade/salt-okunur listelerdi) `Detaylı`/`Profesyonel`/`Kolay` ile aynı güce kavuşsun — çoklu seçim + toplu işlem + tam görev kartı (düzenleme, alt görevler, sonsuz iç içe, Nested Promote, katla, rozet, tüm menü aksiyonları). Aydınlık için **Seçenek 1** seçildi: beyaz liste korunur, göreve tıklayınca **koyu TaskCard bir modal içinde** açılır.
**YENİ ortak altyapı (kod tekrarını önlemek için):**
- `lib/useTaskActions.jsx`: TasksPanel/Kolay'daki tüm görev handler setini (status, sil, arşiv, iptal, iş kolu, sabitle, hatırlatıcı, alt görevler/optimistic setSubtasks, promote/demote, kopyala, link) + `cardPropsFor(task)` + `modalsElement` (EditTaskModal + LinkTasksModal) + collapse state + reminderConfig fetch tek yerde toplar. isTeamView=false (kişisel "mine" listesi). Detaylı/Kolay/Profesyonel'e DOKUNULMADI.
- `components/tasks/TaskCardModal.jsx`: tam `TaskCard`'ı portal + koyu backdrop modal içinde açar. Teknik ve Aydınlık ortak kullanır.
**Teknik** (`TeknikInterface.jsx`): tabloya # ve ALT (x/y) sütunları; `SEÇ` komutu → yapışkan `TaskBulkBar` (Tümünü Seç/Tamamla/Beklet/Aktif/Tarihi geçti/Arşivle/İş Koluna Taşı/Sabitle/Sabiti kaldır/Sil); satır kutucuğu ([x]/[ ]); satır/[AÇ] tıklayınca koyu TaskCard modalı; sağ detay panelinde "TAM KARTI AÇ". `useTaskBulk` + `useTaskActions` + `computeTaskNumbers` bağlandı; groups (link için) yüklenir.
...
## ✅ HATA RADARI TREND — SEVİYE KIRILIMI (2026-06 · fork) TAMAMLANDI
Kullanıcı isteği: Trend grafiğinde her günü Hata/Uyarı olarak iki renge ayır.
- **Backend** (`GET /admin/client-logs` · `daily`): her güne `errors` (error/critical/fatal) ve `warnings` (warning/warn) alanları eklendi — `$group` içinde `$cond`+`$in`+`$toLower/$ifNull` ile seviye sayımı. `count` toplam olarak korundu.
- **Frontend** (`ClientErrorRadar.jsx` · `ErrorTrendChart`): tek renkli çubuklar **yığılmış (stacked) çubuğa** dönüştü — alt: Hata (kırmızı/danger), üst: Uyarı (turuncu), varsa kalan: nötr gri. Bugün artık renk yerine **cyan halka (ring)** ile vurgulanır (seviye renkleriyle çakışmasın). Altına **açıklama (legend)**: 🟥 Hata · 🟧 Uyarı. Tooltip: "GÜN · N hata · M uyarı".
- **Test:** `daily` API errors/warnings kırılımını doğru döndürdü (ör. 09-16: 3 hata + 2 uyarı). İzole `client_logs`e seviye-karışık 15 sahte kayıt eklendi → yığılmış çubuklar + cyan bugün halkası + legend doğru render oldu (screenshot). 15 kayıt KALICI silindi. CRA build temiz. Üretim verisine dokunulmadı.

## ✅ HATA RADARI TREND — GÜN TIKLAMA FİLTRESİ (2026-06 · fork) TAMAMLANDI
Kullanıcı isteği: Trend grafiğinde bir güne tıklayınca alttaki hata listesi o güne süzülsün.
- **Backend** (`GET /admin/client-logs`): opsiyonel `day=YYYY-MM-DD` parametresi eklendi — `created_at` ISO string'ini o UTC gününe daraltır (`$gte T00:00:00` / `$lt ertesi gün`). Sadece liste sorgusunu (`q`) etkiler; `daily`/`total`/`active`/`last_24h` global kalır (çubuklar sabit).
- **Frontend** (`ClientErrorRadar.jsx`): `day` state; `clientLogsApi.list` `day` parametresi. `ErrorTrendChart` çubukları artık tıklanabilir buton — count>0 günler tıklanır, seçili gün ring-2 cyan + hafif cyan zemin ile vurgulanır; aynı güne tekrar tıklama filtreyi kaldırır (toggle). Liste başlığında `16 Eyl ✕` chip'i (temizle) + gün-duyarlı boş mesaj ("… için kayıt yok"). `dayLabel` yardımcı fn eklendi.
- **Test:** API `day` süzmesi doğrulandı (all=15 → day=2026-09-16 → 5, hepsi aynı tarih). UI: ÇAR çubuğuna tıklama 15→5 satır, "16 EYL" chip'i, temizle → 15 (screenshot + satır sayımıyla doğrulandı). İzole `client_logs`e 15 sahte kayıt eklenip KALICI silindi; üretim verisine dokunulmadı. CRA build temiz.

## ✅ HATA RADARI — KAYNAĞA GÖRE GRUPLAMA (2026-06 · fork) TAMAMLANDI
Kullanıcı isteği: Hata listesini oluştuğu dosya/kaynak (source) bazında da grupla.
- **Backend** (`POST /admin/client-logs/resolve-bulk`): `ClientLogResolveBulk` modeline `source: Optional[str]` + `by: str="message"` eklendi. `by=="source"` ise `{"source": src}` (boş/None → `{"source":{"$in":[None,""]}}` = "kaynak yok" grubu), aksi halde eski mesaj davranışı. Grup çözümü artık kaynağa göre de toplu çalışır.
- **Frontend** (`ClientErrorRadar.jsx`): görünüm ikili (`grouped` bool) → 3'lü `groupMode` ("none"|"message"|"source"). Çubuk: `[Liste] [Mesaj] [Kaynak]`. `groupLogs(logs, mode)` genelleştirildi (key/label; source için `l.source||""`, etiket "(kaynak yok)"). Grup başlığı source modunda FileCode2 ikonu + kaynak yolu; açılınca alt satırda mesaj metni gösterilir. `resolveGroup` `by/source` gönderir. Başlık: "KAYNAĞA GÖRE (SIK → SEYREK)". `clientLogsApi.resolveBulk` obje imzasına geçti (`{by,message,source,resolved}`).
- **Test:** `resolve-bulk by=source` doğrulandı (api.js → 4 kayıt çözüldü). İzole `client_logs`e 4 farklı kaynağa (app.js/TaskCard.jsx/api.js/None) dağıtılmış 15 sahte kayıt eklendi → KAYNAK modunda gruplar doğru render oldu (app.js ×4, FileCode2, Çöz(4); "KAYNAĞA GÖRE" başlığı; screenshot). 15 kayıt `e2e` işaretiyle KALICI silindi; üretim verisine dokunulmadı. CRA build temiz.

## ✅ HATA RADARI — EN ÇOK HATA ÜRETEN KAYNAKLAR (2026-06 · fork) TAMAMLANDI
Kullanıcı isteği: Trend grafiğinin altına en çok hata üreten ilk 3 kaynağı mini çubuklarla göster.
