...
- **Bekleyen (backlog, YAPMA — sadece not) — DÜZELTİLDİ 2026-06 (görselli)**: Overlap sorunu **PROFESYONEL temada** (Detaylı değil). Sağdan **NEURAL LINK** paneli açılınca panel `absolute/fixed` overlay gibi ana içeriğin ÜSTÜNE biniyor; özellikle sağdaki "Son Görevler" ve "Yaklaşan Son Tarihler" kartları panelin altında kalıp kesiliyor. Ana ızgara panele yer açmak için daralmıyor (reflow yok). ÇÖZÜM planı: Profesyonel temada panel açıkken ana içerik konteynerine panel genişliği kadar sağ boşluk (margin/padding-right) ver / grid'i daralt → Detaylı'daki çalışan reflow mantığının aynısı. Kullanıcı "yapma bekle" dedi — "YAP" deyince düzeltilecek.
